...
16. **归档导入 UI 生命周期（2026-09-09）**：`archive-import` 是 OpenClaw 存储升级后的静态历史补录，不按同步周期标记 stale；API 将其标记为 `kind: archive`、`status: fresh`。它继续计入 All-source 总统计并保留来源筛选入口，但不在面向持续同步源的「来源概览」中展示。
